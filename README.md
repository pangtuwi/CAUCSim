...
- **Paginated Results Panel:** Simulation results are organized into three switchable pages — Aerodynamics Graphs, Centreline Flow, and Streamlines — via a tabbed interface, keeping each visualisation fully in view instead of stacking them into one long scrolling panel.
...
- **3D Streamlines Visualisation:** Renders an isometric 3D streamtracer scene (colored by velocity magnitude) from the OpenFOAM `streamlines` function object's track output, using headless ParaView (`pvpython`) on the droplet. Produces both a static PNG thumbnail and an interactive GLTF model (buffers inlined as base64, single self-contained file) that loads directly into the main Three.js viewport, toggleable alongside the car geometry. Requires `pvpython` to be pre-installed on the DigitalOcean droplet snapshot; the step is skipped gracefully (without failing the job) if it isn't available.
