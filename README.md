...
- **Surface Pressure (Cp) Visualisation:** Colors the car body surface by pressure coefficient using a diverging scale, exported as GLTF via `foamToVTK` + ParaView (`pvpython`), with a legend showing the run's actual Cp range. Toggling it on temporarily replaces the Shaded/Wire/Points view (they'd otherwise z-fight) and restores the prior view when toggled off. Disabled until that run's pressure data is available.
