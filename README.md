...
- **Surface Pressure (Cp) Visualisation:** Colors the car body surface by pressure coefficient using a diverging scale, exported as GLTF via `foamToVTK` + ParaView (`pvpython`). The color scale (and its legend) is clipped to the 1st-99th percentile of the run's Cp values rather than the raw min/max, so a handful of numerically noisy outlier cells (common on a non-converged solve or a sharp-edged mesh cell) can't dominate the legend and wash out real variation across the rest of the surface; those outliers still render, just clamped to the nearest end of the scale. Toggling it on temporarily replaces the Shaded/Wire/Points view (they'd otherwise z-fight) and restores the prior view when toggled off. Disabled until that run's pressure data is available.
